...
class Mflare_Dataset(torch.utils.data.Dataset):
    def __init__(self, img_path, seq_path, csv, transform):
    ...
    def __getitem__(self, idx):
        ...
        #print(seq)
        #seq_imgs = torch.empty(size=(len(seq_list), 3, 256, 256))
...
train1 = '../Seq16_flare_Mclass_48h_TrainVal' + str(n_train1) + '.txt '
train2 = '../Seq16_flare_Mclass_48h_TrainVal' + str(n_train2) + '.txt '
train3 = '../Seq16_flare_Mclass_48h_TrainVal' + str(n_train3) + '.txt '
train4 = '../Seq16_flare_Mclass_48h_TrainVal' + str(n_train4) + '.txt '
val = '../Seq16_flare_Mclass_48h_TrainVal' + str(n_val) + '.txt'

os.system('cat ' + train1 + train2 + train3 + train4 + ' > Seq16_flare_Mclass_48h_train3.txt')
os.system('cat ' + val + ' > Seq16_flare_Mclass_48h_val3.txt')

# In[ ]:

train_dataset = Mflare_Dataset('../../../magnetogram_jpg/','../Seqs16/','Seq16_flare_Mclass_48h_train3.txt', transform=transform_train)
val_dataset = Mflare_Dataset('../../../magnetogram_jpg/','../Seqs16/','Seq16_flare_Mclass_48h_val3.txt', transform=transform_valid)
test_dataset = Mflare_Dataset('../../../magnetogram_jpg/','../Seqs16/','../Seq16_flare_Mclass_48h_Test.txt', transform=transform_valid)
...
net_name = net_name + '_S_10-7' #_aggB'
...
flaresRAsTrain = pd.read_csv('Seq16_flare_Mclass_48h_train3.txt', sep=' ', dtype='str')
flaresRAsVal = pd.read_csv('Seq16_flare_Mclass_48h_val3.txt', sep=' ', dtype='str')
flaresRAsTest = pd.read_csv('../Seq16_flare_Mclass_48h_Test.txt', sep=' ', dtype='str')
...
trainer = Trainer(callbacks=[checkpoint_callback, early_stop_callback, lr_monitor],
                  max_epochs=hparams['max_epochs'], accelerator='gpu', devices=2, strategy='ddp') #accumulate_grad_batches=10')
...
# In[ ]:
trainer_test = Trainer(max_epochs=hparams['max_epochs'], accelerator='gpu', devices=1)


trainer_test.test(test_model, test_dataloader)
